...
class CoreDB(JsonGuildDB):
    """
    The central DB used by Joan to store a variety
    of settings, both global and guild specific
    """

    def can_login(self):
        """Used on start to determine if Joan is setup enough to login"""
    ...
    async def set_guild_blacklist(self, guild, blacklist):
        ...
        if not isinstance(guild, discord.Guild) or not isinstance(blacklist, list):
            raise TypeError("A valid Discord guild or a list of IDs "
                            "must be passed.")
